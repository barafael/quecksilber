...
use iced::{mouse, Element, Font, Length, Point, Rectangle, Renderer, Theme};
...
impl Gauge {
    /// Create a new gauge with the given range and current value.
    ...
    pub fn draw_at(
        ...
        #[derive(PartialEq)]
        enum TickKind { Label, Mid, Small }

        let mut ticks: Vec<(f32, TickKind)> = Vec::new();
        ...
        for (val, kind) in &ticks {
            ...
            if show_label {
                ...
                let label_pos = Point::new(
                    center.x + cos * label_r,
                    center.y + sin * label_r,
                );

                let text = Text {
                    content: format!("{}", *val as i32),
                    position: label_pos,
                    size: (radius * 0.2 * density_scale).into(),
                    color: fg,
                    font: self.font,
        ...
        match self.arm_style {
            ...
            ArmStyle::Needle => {
                ...
                let arm = Path::new(|b| {
                    let tail_end = Point::new(
                        center.x - cos * tail_length,
                        center.y - sin * tail_length,
                    );
                    // Rectangle corners
        ...
        if !self.upper_label.is_empty() {
            ...
            let upper_text = Text {
                content: self.upper_label.clone(),
                position: upper_pos,
                size: (radius * 0.18).into(),
                color: fg,
                font: self.font,
